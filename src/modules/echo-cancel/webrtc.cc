...
bool pa_webrtc_ec_init(pa_core *c, pa_echo_canceller *ec,
    ...
    pa_log("Start webrtc init function...");
...
#define IS_ZERO(f) ((f) < 0.000001 && (f) > -0.000001)
    ...
    *nframes = ec->params.webrtc.blocksize;
    ...
    pa_modargs_free(ma);
    pa_log("Finish webrtc init function...");
    return true;
...
void pa_webrtc_ec_run(pa_echo_canceller *ec, const uint8_t *rec, const uint8_t *play, uint8_t *out) {
    pa_log("Start runnning webrtc ec...");
    pa_webrtc_ec_play(ec, play);
    pa_webrtc_ec_record(ec, rec, out);
    pa_log("Finished runnning webrtc ec");
}

void pa_webrtc_ec_done(pa_echo_canceller *ec) {
    int i;

    pa_log("Start webrtc done function...");
